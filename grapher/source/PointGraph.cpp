...
void PointGraph::draw(QMatrix4x4 & view,
					  ...
					  float zoomFactor)
{
	...
	for(size_t i = 0; i < vertexBuffers_.size() ; ++i)
	{
		...
		else
		{
			 ...
			 */
			current->bind();
			glDrawElements(GL_POINTS, current->getNumberOfIndices(), GL_UNSIGNED_INT, (void*)0);
			current->release();
		}
...
void PointGraph::generateIndicesAndRegenerateVertexBuffer()
{
	/**
