...
#export LD_LIBRARY_PATH=${MB_DIR}/usr/lib:${MB_DIR}/usr:${LD_LIBRARY_PATH}
...
./configure --prefix=${MB_DIR}/usr/ --extra-cflags="-I${MB_DIR}/usr/include" --extra-ldflags="-L${MB_DIR}/usr/lib" --enable-libmp3lame --enable-shared --enable-gpl --enable-pthreads --arch=x86_64 --enable-ssse3 --disable-debug  # --cc=clang
make
make install
...
export PKG_CONFIG_PATH=${MB_DIR}/usr/lib/pkgconfig
