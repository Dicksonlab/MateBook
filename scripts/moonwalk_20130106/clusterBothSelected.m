function clusterBothSelected(gatheredResultsFile)
	attributes = {...
        'total movement',...
	...
	videoFiles = videoFiles(videosToKeep);
	videoCount = size(stats, 1);
    videoCount=videoCount-1; %wierd case, check later
    tree=linkage(stats,'ward','euclidean','savememory','on');
%     tree2=clusterdata(stats,'maxclust',10,'distance','euclidean','linkage','ward');
    for i =[0:(videoCount-1)]
        X(videoCount-i)=i;
    end
    figure;
    plot(X,tree(:,3))
    figure;
    plot(X(videoCount-50:videoCount-4),tree(videoCount-50:videoCount-4,3))
    figure;
    plot(X(videoCount-50:videoCount-4),diff(tree(videoCount-50:videoCount-3,3)))
%     plot(X(375:473),diff(tree(375:end,3)))
    figure;
	[~, nodeNumbers] = dendrogram(tree,0,'colorthreshold',28.9);
...
...%		'RowLabels', {text{2:end,1}},...
    ...
    set(cg,'Standardize',3);
    set(cg,'DisplayRange',5);
