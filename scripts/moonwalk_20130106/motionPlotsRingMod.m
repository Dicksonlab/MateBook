function motionPlotsRingMod(inDirectory, outDirectory)
	...
	stateThreshold = [-0.2 1];	% if the fly moves slower than that (in mm/s) it will be counted as a stall
	stateChangePenalty = [2,2]; % in mm; the t-score we feed into the hofacker when determining the globally best locomotion state assignment
	...
	for entry = listing'
		...
		if isdir([inDirectory '/' entry.name])
			...
			if success
				motionPlotsRingMod([inDirectory '/' entry.name], [outDirectory '/' entry.name]);
			else
