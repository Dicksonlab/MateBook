...
#define NEW_FLY_ATTRIBUTE(group, type, name, description, unit) attributeMap[STRINGIFY(name)] = new Attribute<type>(STRINGIFY(name), description, unit); group.push_back(STRINGIFY(name));
...
#else
	#define NEW_TRACKING_ATTRIBUTE(group, type, name, description, unit) attributeMap[STRINGIFY(name)] = new TrackingAttributeFly<type>(STRINGIFY(name), description, unit, &Fly::get_##name); group.push_back(STRINGIFY(name));
#endif

FlyAttributes::FlyAttributes() : AttributeCollection()
{
	NEW_TRACKING_ATTRIBUTE(trackingAttributes, uint32_t, bodyContourOffset, "Offset into the contour file to access the body contour.", "");
	NEW_TRACKING_ATTRIBUTE(trackingAttributes, uint32_t, bodyContourSize, "Number of pixels that are part of the body contour.", "");
	NEW_TRACKING_ATTRIBUTE(trackingAttributes, uint32_t, wingContourOffset, "Offset into the contour file to access the wing contour.", "");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, Vf2, bodyCentroid, "Centroid of the ellipse fit to the fly body.", "px");
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, bodyMajorAxisLength, "Major axis length of the ellipse fit to the fly body.", "px");
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, bodyMinorAxisLength, "Minor axis length of the ellipse fit to the fly body.", "px");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, bodyOrientation, "Global orientation of the ellipse fit to the fly body, after reinterpolating using the final heading.", "deg");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, bodyEccentricity, "Eccentricity of the ellipse fit to the fly body.", "");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, movedDirectionLocal, "Local motion direction since the last frame.", "deg");
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, turned, "The change of orientation since the last frame.", "deg");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, Vf2, leftWingTip, "Actual tip of the left wing.", "px");
	NEW_FLY_ATTRIBUTE(derivedAttributes, Vf2, rightWingTip, "Actual tip of the right wing.", "px");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, leftWingArea, "Actual area of the left wing.", "px");
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, rightWingArea, "Actual area of the right wing.", "px");
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, leftWingAngle, "Actual angle of the left wing.", "deg");
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, rightWingAngle, "Actual angle of the right wing.", "deg");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, MyBool, wingExtLeft, "Left wing extended.", "");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, MyBool, wingExtRight, "Right wing extended.", "");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, MyBool, wingExtBoth, "Both wings extended.", "");
	NEW_FLY_ATTRIBUTE(derivedAttributes, MyBool, wingExtEitherOr, "Exactly one wing extended.", "");
	NEW_FLY_ATTRIBUTE(derivedAttributes, MyBool, copulating, "Fly is copulating.", "");	// same for all flies for now - tricky because it's based on a frameAttribute
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, MyBool, courting, "Fly is courting.", "");	// tricky because it's based on a fly-attribs (wingExt) and pair-attribs; we should add a courting pair-attrib in the future

	// attributes resulting from unit conversion
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, bodyAreaEccentricityCorrected_u, "Fly body area after correcting for posture.", "mm²");
	NEW_FLY_ATTRIBUTE(derivedAttributes, Vf2, bodyCentroid_u, "Centroid of the ellipse fit to the fly body.", "mm");
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, bodyMajorAxisLength_u, "Major axis length of the ellipse fit to the fly body.", "mm");
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, bodyMinorAxisLength_u, "Minor axis length of the ellipse fit to the fly body.", "mm");
	...
	NEW_FLY_ATTRIBUTE(derivedAttributes, float, movedAbs_u, "The distance moved since the last frame.", "mm");
}
